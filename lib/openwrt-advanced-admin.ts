...
export type ProxyServiceId = "openclash" | "adguardhome";
export type ManagedAction = "start" | "stop" | "restart";
export type RouterLogCategory = "system" | "kernel" | "dns" | "dial" | "firewall";
...
const PROXY_SERVICES = [
  { id: "openclash", label: "OpenClash", initName: "openclash", processHint: "clash" },
  { id: "adguardhome", label: "AdGuard Home", initName: "AdGuardHome", processHint: "AdGuardHome" },
] as const;
...
function requireFirewallSection(value: string) {
  const normalized = value.trim();
  if (/^[A-Za-z0-9_-]+$/.test(normalized) || /^@redirect\[\d+\]$/.test(normalized)) return normalized;
...
function requirePortSpec(value: string, label: string) {
  ...
  if (start < 1 || end > 65535 || start > end) throw new Error(`${label}范围应为 1–65535。`);
  return normalized;
}
...
export function buildProxyServiceSnapshotCommand() {
  return PROXY_SERVICES.map((service) => (
...
export function parseProxyServiceStates(output: string): ProxyServiceState[] {
  const states = new Map<ProxyServiceId, ProxyServiceState>(PROXY_SERVICES.map((service) => [service.id, {
    id: service.id,
    label: service.label,
  ...
  for (const line of output.split(/\r?\n/)) {
    const match = line.trim().match(/^PROXY\|(openclash|adguardhome)\|(installed|missing)\|(running|stopped)\|([A-Za-z0-9_.-]+)$/);
    if (!match) continue;
    const [, rawId, availability, state, initName] = match;
    const id = rawId as ProxyServiceId;
    const current = states.get(id);
    if (!current) continue;
    states.set(id, { ...current, initName, installed: availability === "installed", running: state === "running" });
  }
  return PROXY_SERVICES.map((service) => states.get(service.id)!).filter(Boolean);
}

export function buildProxyServiceActionCommand(id: ProxyServiceId, action: ManagedAction) {
...
export function parseHealthSnapshot(output: string): RouterHealthSnapshot {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (line === "__DISKS__") { section = "disks"; continue; }
    if (line === "__TEMPERATURES__") { section = "temperatures"; continue; }
    if (line === "__PING__") { section = "ping"; continue; }
    if (line === "__DNS__") { section = "dns"; continue; }
    if (!line) continue;
    if (section === "disks") {
      const match = line.match(/^DISK\|([^|]+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)%$/);
      if (match) disks.push({ mount: match[1], totalKb: safeNumber(match[2]), usedKb: safeNumber(match[3]), availableKb: safeNumber(match[4]), usePercent: safeNumber(match[5]) });
    } else if (section === "temperatures") {
      const match = line.match(/^TEMP\|(-?\d+(?:\.\d+)?)$/);
      if (match) {
        const raw = Number(match[1]);
        const celsius = Math.abs(raw) > 200 ? raw / 1000 : raw;
        if (Number.isFinite(celsius) && celsius > -50 && celsius < 150) temperaturesC.push(Math.round(celsius * 10) / 10);
      }
  ...
  const packet = pingOutput.match(/(\d+)\s+packets? transmitted,\s*(\d+)\s+(?:packets? )?received.*?(\d+(?:\.\d+)?)%\s*packet loss/i);
  const average = pingOutput.match(/=\s*[\d.]+\/([\d.]+)\/[\d.]+(?:\/[\d.]+)?\s*ms/i);
  const ping = packet ? {
  ...
  const dnsReachable = !dnsOutput ? null : !/(connection refused|server can't find|not found|failed|timed out|no servers could be reached)/i.test(dnsOutput) && /(?:address|name):/i.test(dnsOutput);
  return { disks, temperaturesC: [...new Set(temperaturesC)], ping, dnsReachable };
}

export function buildRouterLogCommand(category: RouterLogCategory, limit = 160, filter = "") {
  const safeLimit = Math.min(400, Math.max(20, Math.floor(limit)));
  const query = filter.trim();
  if (query.length > 80 || /[\r\n]/.test(query)) throw new Error("日志筛选词最多 80 个字符，且不能包含换行。");
...
function readFirewallSections(output: string) {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    const line = rawLine.trim();
    if (line === "__FIREWALL__") { firewallSection = true; continue; }
    if (line === "__UPNP__") { firewallSection = false; continue; }
    if (!firewallSection) continue;
    const match = line.match(/^firewall\.((?:@)?[A-Za-z0-9_-]+(?:\[\d+\])?)(?:\.([A-Za-z0-9_]+))?=(.*)$/);
    if (!match) continue;
...
function firstValue(section: UciSection, property: string, fallback = "未设置") {
...
export function parseFirewallSnapshot(output: string): FirewallSnapshot {
  ...
  for (const [section, value] of sections) {
    if (value.type === "zone") {
      zones.push({ section, name: firstValue(value, "name", section), networks: value.values.get("network") ?? [], input: firstValue(value, "input"), output: firstValue(value, "output"), forward: firstValue(value, "forward") });
    }
    if (value.type === "redirect") {
      portForwards.push({ section, name: firstValue(value, "name", section), sourceZone: firstValue(value, "src"), destinationZone: firstValue(value, "dest"), destinationIp: firstValue(value, "dest_ip"), sourcePort: firstValue(value, "src_dport"), destinationPort: firstValue(value, "dest_port"), protocol: firstValue(value, "proto", "tcp udp"), enabled: firstValue(value, "enabled", "1") !== "0" });
    }
  }
  const upnpMatch = output.match(/^UPNP\|(installed|missing)\|(running|stopped)\|([^\r\n|]+)$/m);
  return {
    zones: zones.sort((a, b) => a.name.localeCompare(b.name)),
    portForwards: portForwards.sort((a, b) => a.name.localeCompare(b.name)),
    upnp: { installed: upnpMatch?.[1] === "installed", running: upnpMatch?.[2] === "running", enabled: upnpMatch?.[3] === "1" },
  };
}

export function buildPortForwardToggleCommand(section: string, enabled: boolean) {
...
export function buildPortForwardCreateCommand(draft: PortForwardDraft) {
  ...
  if (!name || name.length > 48 || /[\r\n]/.test(name)) throw new Error("规则名称应为 1–48 个字符，且不能包含换行。");
  ...
  if (!["tcp", "udp", "tcp udp"].includes(draft.protocol)) throw new Error("端口协议无效。");
...
export function buildBatchConfigBackupCommand(batchId: string) {
  ...
  return { remotePath, command: `rm -f ${shellQuote(remotePath)}; sysupgrade -b ${shellQuote(remotePath)}; test -s ${shellQuote(remotePath)} && echo 'BACKUP_READY'` };
}
...
export function buildRouterHealthReportMarkdown(profile: RouterProfile, status: RouterStatus | null, health: RouterHealthSnapshot | null, services: ProxyServiceState[] = []) {
  const system = status?.system ?? null;
  const memoryPercent = memoryUsagePercent(system);
  const disks = health?.disks.length ? health.disks.map((disk) => `${disk.mount}: ${disk.usePercent ?? "—"}%（可用 ${disk.availableKb === null ? "—" : formatBytes(disk.availableKb * 1024)}）`).join("；") : "未报告";
  const temperature = health?.temperaturesC.length ? health.temperaturesC.map((value) => `${value} °C`).join("、") : "未报告";
  const ping = health?.ping ? `${health.ping.lossPercent ?? "—"}% 丢包，平均 ${health.ping.averageMs ?? "—"} ms` : "未报告";
  const servicesText = services.length ? services.map((service) => `${service.label}：${service.installed ? (service.running ? "运行中" : "已停止") : "未安装"}`).join("；") : "未检测";
