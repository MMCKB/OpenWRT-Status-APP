...
export default function ClientsScreen() {
          ...
          onPress: () =>
            void (async () => {
              try {
                await execute(
                  isBlocked
                    ? buildUnblockClientCommand(client.mac)
                    : buildBlockClientCommand(client.mac),
                );
                await refresh();
              } catch {}
            })(),
  ...
  return (
    <ManagementShell
      title="已连接设备"
      description="设备来源于 DHCP 租约和邻居表；在线状态反映最近一次路由器查询。网络唤醒已移至工具页。"
    >
      <View style={styles.toolbar}>
        <StatusPill label={`${clients.length} 台设备`} tone="normal" />
        <Pressable
      ...
      {!isSupported ? (
        <ToolNotice>
          <Text style={[styles.noticeText, { color: colors.muted }]}>
            设备扫描需要安装最新 Android APK，因为它会通过应用内 SSH
            查询路由器。
          </Text>
        </ToolNotice>
      ) : null}
      ...
      {clients.length ? (
          ...
          {clients.map((client, index) => (
            ...
            >
              ...
              <View style={styles.clientInfo}>
                <Text
                  style={[styles.clientTitle, { color: colors.foreground }]}
                >
              ...
              <View style={styles.clientActions}>
                <Pressable
                  accessibilityRole="button"
                  onPress={() => changeBlock(client)}
                  disabled={isRunning}
                  style={({ pressed }) => [
                    styles.blockButton,
                    {
                      borderColor: blocked.has(client.mac)
                        ? colors.success
                        : colors.border,
                    },
                    pressed && styles.pressed,
                ...
                >
                  <Text
                    style={[
                      styles.blockText,
                      {
                        color: blocked.has(client.mac)
                          ? colors.success
                          : colors.foreground,
                      },
                    ]}
                  >
                    {blocked.has(client.mac) ? "解除" : "拉黑"}
                  </Text>
                </Pressable>
              </View>
            </View>
        ...
        <EmptyState
          icon="devices"
          title="暂未读取到客户端"
          description="请点击刷新。部分静态 IP 设备不会出现在 DHCP 租约中。"
        />
      ) : null}
    </ManagementShell>
  );
...
const styles = StyleSheet.create({
  ...
  clientTitle: { fontSize: 14, fontWeight: "800" },
  clientSub: { fontSize: 11, marginTop: 3, fontVariant: ["tabular-nums"] },
  clientActions: { flexDirection: "column", gap: 6, alignItems: "stretch" },
  blockButton: {
    minWidth: 50,
    paddingHorizontal: 9,
    minHeight: 32,
