...
export default function RouterFormScreen() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setTested("error");
      setMessage(
        error instanceof Error ? error.message : "无法保存路由器资料。",
      );
    } finally {
      setIsSaving(false);
    }
    ...
    Alert.alert(
      ...
      [
        { text: "取消", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: () => {
            void deleteProfile(existing.id).then(() =>
              router.replace("/routers" as never),
            );
          },
        },
      ],
      ...
      >
        <View style={styles.nav}>
          ...
          <Text style={[styles.navTitle, { color: colors.foreground }]}>
            {existing ? "编辑路由器" : "添加路由器"}
          </Text>
          <View style={styles.navSpacer} />
        </View>
        <View>
          <Text style={[styles.title, { color: colors.foreground }]}>
            {existing ? "连接资料" : "连接 OpenWrt"}
          </Text>
          <Text style={[styles.subtitle, { color: colors.muted }]}>
            使用 LuCI 的 ubus 接口读取系统与网络状态。
          </Text>
        </View>
        <View
          style={[
        ...
        >
          ...
          <TextInput
            ...
            placeholder="http://192.168.1.1 或完整 /ubus 地址"
            placeholderTextColor={colors.muted}
            selectionColor={colors.primary}
            autoCapitalize="none"
            autoCorrect={false}
          ...
          />
          <Text style={[styles.helpText, { color: colors.muted }]}>
            应用内终端使用 LuCI 地址中的主机名，以及这里设置的 SSH 账户与端口。
          </Text>
          <Text style={[styles.fieldLabel, { color: colors.foreground }]}>
            密码{existing ? "（留空以保留原密码）" : ""}
          </Text>
          <View style={styles.passwordField}>
            <TextInput
              ...
              placeholder={existing ? "输入新密码以替换" : "LuCI 密码"}
              placeholderTextColor={colors.muted}
              selectionColor={colors.primary}
            ...
            <Pressable
              accessibilityRole="button"
              accessibilityLabel={
                isPasswordVisible ? "隐藏 LuCI 密码" : "显示 LuCI 密码"
              }
          ...
          <Text style={[styles.fieldLabel, { color: colors.foreground }]}>
            SSH 密码
            {existing ? "（留空以保留原密码）" : "（留空则使用 LuCI 密码）"}
          </Text>
          <View style={styles.passwordField}>
            <TextInput
              style={[
              ...
              placeholderTextColor={colors.muted}
              selectionColor={colors.primary}
              secureTextEntry={!isSshPasswordVisible}
              autoCapitalize="none"
              autoCorrect={false}
              returnKeyType="done"
              onSubmitEditing={() => void handleSave()}
            ...
            <Pressable
              accessibilityRole="button"
              accessibilityLabel={
                isSshPasswordVisible ? "隐藏 SSH 密码" : "显示 SSH 密码"
              }
              accessibilityState={{ selected: isSshPasswordVisible }}
              hitSlop={8}
              onPress={() => setIsSshPasswordVisible((visible) => !visible)}
              style={({ pressed }) => [
                styles.passwordToggle,
                pressed && styles.buttonPressed,
              ]}
            >
              <MaterialIcons
                name={isSshPasswordVisible ? "visibility-off" : "visibility"}
                size={21}
                color={colors.muted}
              />
            </Pressable>
        ...
        {message ? (
          <View style={[styles.message, { backgroundColor: messageSurface }]}>
            <StatusPill
              label={tested === "error" ? "连接失败" : "连接成功"}
              tone={tested === "error" ? "danger" : "success"}
            />
        ...
        >
          {isSaving ? (
            ...
            <Text style={sharedStyles.primaryButtonText}>保存路由器</Text>
          )}
        </Pressable>
        {existing ? (
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="删除路由器"
