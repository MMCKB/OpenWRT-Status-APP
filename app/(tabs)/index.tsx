...
export default function StatusScreen() {
  ...
  return (
    <View style={[sharedStyles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={sharedStyles.content} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => void refreshStatus()} tintColor={colors.primary} />}>
        <View style={styles.headerRow}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="刷新状态" onPress={() => void refreshStatus()} style={({ pressed }) => [styles.refreshButton, { backgroundColor: softPrimary }, pressed && styles.iconPressed]}><MaterialIcons name="refresh" size={22} color={colors.primary} /></Pressable>
        </View>
        ...
        <RealtimeTrafficCard interfaces={selectedStatus?.interfaces ?? []} fetchedAt={selectedStatus?.fetchedAt} refreshing={settings.refreshIntervalSeconds === 1 ? isTrafficRefreshing : isRefreshing} refreshIntervalSeconds={settings.refreshIntervalSeconds} selectedInterfaceIds={settings.trafficInterfaceIds} viewMode={settings.statusTrafficView} />
        ...
        <SectionCard title="无线网络" action={<Text style={[styles.sectionMeta, { color: colors.muted }]}>{selectedStatus?.wireless.length ?? 0} 个</Text>}>
