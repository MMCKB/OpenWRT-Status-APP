...
const themeOptions: { label: string; value: ThemePreference; icon: React.ComponentProps<typeof MaterialIcons>["name"] }[] = [
    ...
    <View style={[sharedStyles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={sharedStyles.content}>
        ...
        <SectionCard title="外观">
          <Text style={[styles.cardDescription, { color: colors.muted }]}>主题仅影响本应用。选择“跟随系统”时，将自动使用设备当前的深浅色外观。</Text>
          <View style={styles.themeGrid}>{themeOptions.map((option) => {
            const selected = themePreference === option.value;
            return <Pressable key={option.value} accessibilityRole="button" accessibilityState={{ selected }} onPress={() => void setThemePreference(option.value)} style={({ pressed }) => [styles.themeOption, { borderColor: selected ? colors.primary : colors.border, backgroundColor: selected ? colors.primary : colors.surface }, pressed && styles.pressed]}><MaterialIcons name={option.icon} size={19} color={selected ? "#FFFFFF" : colors.primary} /><Text style={[styles.themeLabel, { color: selected ? "#FFFFFF" : colors.foreground }]}>{option.label}</Text></Pressable>;
          })}</View>
        </SectionCard>
...
function InfoRow({ icon, title, description, colors, softPrimary, divider = false }: { icon: React.ComponentProps<typeof MaterialIcons>["name"]; title: string; description: string; colors: ReturnType<typeof useColors>; softPrimary: string; divider?: boolean }) {
  return <View style={[styles.infoRow, divider && { borderTopWidth: 1, borderTopColor: colors.border }]}><View style={[styles.infoIcon, { backgroundColor: softPrimary }]}><MaterialIcons name={icon} size={19} color={colors.primary} /></View><View style={styles.infoText}><Text style={[styles.infoTitle, { color: colors.foreground }]}>{title}</Text><Text style={[styles.infoDescription, { color: colors.muted }]}>{description}</Text></View></View>;
}

function MaintenanceRow({ icon, label, description, target, colors, softPrimary, router, warning = false }: { icon: React.ComponentProps<typeof MaterialIcons>["name"]; label: string; description: string; target: string; colors: ReturnType<typeof useColors>; softPrimary: string; router: ReturnType<typeof useRouter>; warning?: boolean }) {
  const accent = warning ? colors.warning : colors.primary;
  return <Pressable accessibilityRole="button" accessibilityLabel={`打开${label}`} onPress={() => router.push(target as never)} style={({ pressed }) => [styles.maintenanceRow, pressed && styles.pressed]}><View style={[styles.infoIcon, { backgroundColor: warning ? (colors.background === "#102A43" ? "#553F1E" : "#FFF4DD") : softPrimary }]}><MaterialIcons name={icon} size={19} color={accent} /></View><View style={styles.infoText}><Text style={[styles.infoTitle, { color: colors.foreground }]}>{label}</Text><Text style={[styles.infoDescription, { color: colors.muted }]}>{description}</Text></View><MaterialIcons name="chevron-right" size={20} color={colors.muted} /></Pressable>;
}

const styles = StyleSheet.create({
  title: { fontSize: 28, fontWeight: "800" }, subtitle: { fontSize: 14, marginTop: 5 },
  cardDescription: { fontSize: 14, lineHeight: 20, paddingHorizontal: 15, paddingTop: 15 },
