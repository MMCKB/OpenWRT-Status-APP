  ...
  return <ManagementShell title="代理与健康" description="读取 OpenClash、AdGuard Home 与路由器资源状态；服务控制仅通过应用内 SSH 在本机执行。">
    ...
    <SectionCard title="路由器健康报告" action={<Pressable disabled={!selectedProfile} onPress={() => void exportReport()} style={({ pressed }) => [styles.refresh, { borderColor: colors.border }, pressed && styles.pressed, !selectedProfile && styles.disabled]}><Text style={[styles.refreshText, { color: colors.primary }]}>导出</Text></Pressable>}>
      <View style={styles.metrics}>
        <View style={styles.metricRow}><MetricTile icon="memory" label="内存使用" value={memoryPercent === null ? "未报告" : `${memoryPercent}%`} caption={selectedStatus?.system?.memoryAvailable === null || selectedStatus?.system?.memoryAvailable === undefined ? undefined : `可用 ${formatBytes(selectedStatus.system.memoryAvailable)}`} tone={memoryPercent !== null && memoryPercent >= 90 ? "danger" : memoryPercent !== null && memoryPercent >= 75 ? "warning" : "success"} /><MetricTile icon="storage" label="存储（根/Overlay）" value={disk?.usePercent === null || disk?.usePercent === undefined ? "未报告" : `${disk.usePercent}%`} caption={disk?.availableKb === null || disk?.availableKb === undefined ? undefined : `可用 ${formatBytes(disk.availableKb * 1024)}`} tone={disk?.usePercent !== null && disk?.usePercent !== undefined && disk.usePercent >= 90 ? "danger" : "normal"} /></View>
        <View style={styles.metricRow}><MetricTile icon="thermostat" label="最高温度" value={temperature} caption={health?.temperaturesC.length ? `${health.temperaturesC.length} 个传感器` : "硬件未报告"} tone={health?.temperaturesC.some((value) => value >= 85) ? "danger" : health?.temperaturesC.some((value) => value >= 70) ? "warning" : "normal"} /><MetricTile icon="speed" label="公网延迟" value={health?.ping?.averageMs === null || health?.ping?.averageMs === undefined ? "未报告" : `${health.ping.averageMs} ms`} caption={health?.ping ? `${health.ping.lossPercent ?? "—"}% 丢包 · DNS ${health.dnsReachable ? "正常" : health.dnsReachable === false ? "失败" : "未报告"}` : "采样目标 1.1.1.1"} tone={health?.ping?.lossPercent && health.ping.lossPercent > 0 ? "warning" : health?.ping ? "success" : "normal"} /></View>
      </View>
