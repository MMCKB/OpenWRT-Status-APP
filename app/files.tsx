...
import {
  connectInAppSsh,
  disconnectInAppSsh,
  downloadInAppSshFile,
  getInAppSshTarget,
  isInAppSshSupported,
  runInAppSshCommand,
  uploadInAppSshFile,
  writeInAppSshTextFile,
} from "@/lib/native-ssh";
import {
  buildChmodCommand,
  buildCopyCommand,
...
function fileIcon(entry: RemoteFileEntry) {
  ...
  if (
    /\.(conf|config|json|yaml|yml|txt|log|sh|lua|js|ts|css|html|md)$/i.test(
      entry.name,
    )
  )
    return "description";
...
function describeEntry(entry: RemoteFileEntry) {
  ...
  const details = [
    kind,
    entry.kind === "directory" ? null : formatRemoteSize(entry.size),
    entry.mode ? `权限 ${entry.mode}` : null,
    entry.modifiedAt,
  ].filter(Boolean);
  return details.join(" · ");
}
...
export default function FilesScreen() {
    ...
    return (
      <View style={styles.blankScreen}>
        <View style={styles.nav}>
          ...
          <Text style={styles.navTitle}>文件管理</Text>
          <View style={styles.navSpacer} />
        </View>
        <EmptyState
          icon="folder"
          title="还没有可管理的路由器"
          description="请先在“路由器”中保存 OpenWrt 的 LuCI 与 SSH 连接资料。"
        />
      </View>
    );
  }
  ...
  async function refreshDirectory(directory = currentPath) {
    ...
    try {
      const output = await runInAppSshCommand(
        buildListDirectoryCommand(normalized),
      );
      if (output.includes("__MANUS_NOT_DIRECTORY__"))
        throw new Error("该位置不是可浏览的文件夹。");
      ...
      setMultiSelected([]);
      setNotice(
        `${normalized} · 已加载 ${parseDirectoryEntries(output, normalized).length} 项`,
      );
    } catch (error) {
      setNotice(
        error instanceof Error ? error.message : "无法读取路由器目录。",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function openTextEditor(entry: RemoteFileEntry) {
    ...
    try {
      const result = parseReadableText(
        await runInAppSshCommand(
          buildReadTextCommand(entry.path, TEXT_FILE_LIMIT_BYTES),
        ),
      );
      if (result.tooLargeBytes !== null) {
        Alert.alert(
          "文件过大",
          `${entry.name} 为 ${formatRemoteSize(result.tooLargeBytes)}。为了避免应用内存占用过高，文件编辑器最多打开 ${formatRemoteSize(TEXT_FILE_LIMIT_BYTES)} 的文本文件。`,
        );
  ...
  async function saveTextFile() {
    ...
    try {
      ...
      await runInAppSshCommand(
        buildFinalizeUploadCommand(temporaryPath, editor.entry.path),
      );
      if (editor.entry.mode)
        await runInAppSshCommand(
          buildChmodCommand(editor.entry.path, editor.entry.mode),
        );
      setEditor(null);
      ...
      Alert.alert(
        "保存失败",
        error instanceof Error ? error.message : "无法将修改写入路由器。",
      );
    } finally {
  ...
  async function submitPrompt() {
    ...
    try {
      if (promptKind === "folder") {
        await runInAppSshCommand(
          buildCreateDirectoryCommand(joinRemotePath(currentPath, trimmed)),
        );
      ...
      Alert.alert(
        "操作失败",
        error instanceof Error ? error.message : "无法完成文件操作。",
      );
    }
  }
  ...
  async function chooseAndUpload() {
    ...
    try {
      ...
      Alert.alert(
        "上传文件",
        `将 ${asset.name} 上传到 ${destination}。若目标已存在，将会被替换。`,
        [
          { text: "取消", style: "cancel" },
          {
            text: "上传",
            onPress: () => void uploadAsset(asset.uri, asset.name, destination),
          },
        ],
      );
    } catch (error) {
  ...
  async function uploadAsset(
    ...
    try {
      ...
      await runInAppSshCommand(
        buildFinalizeUploadCommand(temporaryPath, destination),
      );
  ...
  async function downloadAndShare(entry: RemoteFileEntry) {
    ...
    try {
      ...
      if (!cacheDirectory)
        throw new Error("手机缓存目录不可用，无法保存下载文件。");
      const downloadDirectory = `${cacheDirectory}openwrt-downloads/`;
      const safeName =
        entry.name.replace(/[^a-zA-Z0-9._-]/g, "_") || "openwrt-file";
      ...
      if (!(await Sharing.isAvailableAsync())) {
        Alert.alert(
          "文件已下载",
          `已保存到应用缓存：${entry.name}。当前设备无法打开系统分享面板。`,
        );
        return;
      }
      await Sharing.shareAsync(localUri, {
        dialogTitle: `保存或分享 ${entry.name}`,
      });
  ...
  function cycleSortMode() {
    setSortMode((current) =>
      current === "name" ? "size" : current === "size" ? "modified" : "name",
    );
  }

  const activeSelection = isMultiSelecting
    ? multiSelected
    : selected
      ? [selected]
      : [];
  ...
  function stageClipboard(mode: "copy" | "move") {
    ...
    const countLabel =
      activeSelection.length === 1
        ...
        : `${activeSelection.length} 项`;
    setNotice(
      mode === "copy"
        ? `已复制 ${countLabel}，请进入目标文件夹后粘贴。`
        : `已剪切 ${countLabel}，请进入目标文件夹后粘贴。`,
    );
  }

  async function pasteClipboard() {
    if (!clipboard || isBusy) return;
    const eligibleEntries = clipboard.entries.filter(
      (entry) => parentRemotePath(entry.path) !== currentPath,
    );
    ...
    try {
      for (const entry of eligibleEntries) {
        const command =
          clipboard.mode === "copy"
            ? buildCopyCommand(entry.path, currentPath)
            : buildMoveCommand(entry.path, currentPath);
        await runInAppSshCommand(command);
      }
      const countLabel =
        eligibleEntries.length === 1
          ? eligibleEntries[0].name
          : `${eligibleEntries.length} 项`;
      setNotice(
        clipboard.mode === "copy"
          ? `已复制 ${countLabel}。`
          : `已移动 ${countLabel}。`,
      ...
      Alert.alert(
        "粘贴失败",
        error instanceof Error ? error.message : "无法将文件放入当前文件夹。",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function deleteSelection() {
    ...
    setIsLoading(true);
    try {
      for (const entry of activeSelection)
        await runInAppSshCommand(buildDeleteCommand(entry.path));
      setNotice(
        activeSelection.length === 1
          ? `已删除 ${activeSelection[0].name}。`
          : `已删除 ${activeSelection.length} 项。`,
      );
      clearSelection();
      await refreshDirectory();
    } catch (error) {
      Alert.alert(
        "删除失败",
        error instanceof Error ? error.message : "无法删除此文件。\n",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  function goToPath() {
    try {
      ...
      Alert.alert(
        "路径无效",
        error instanceof Error ? error.message : "请输入以 / 开头的路径。",
      );
    }
  }
  ...
  const promptHint =
    promptKind === "folder"
      ...
      : promptKind === "rename"
        ? "输入新名称"
        : "例如 644、755 或 0755";
  const visibleEntries = sortFileEntries(
    filterFileEntries(entries, searchTerm),
    sortMode,
  );
  const sortLabel =
    sortMode === "name" ? "名称" : sortMode === "size" ? "大小" : "时间";
    ...
    >
      <View style={styles.nav}>
        ...
        >
          {isLoading ? (
            ...
            <MaterialIcons name="refresh" size={21} color="#007E7A" />
          )}
        </Pressable>
      </View>
      ...
      {!isInAppSshSupported() ? (
        <View style={styles.platformBanner}>
          <MaterialIcons name="info-outline" size={18} color="#9A6500" />
          <Text style={styles.platformText}>
            文件管理依赖应用内 SSH，仅在重新构建后的 Android APK 中可用。
          </Text>
        </View>
      ) : null}
      ...
      <View style={styles.directorySection}>
        <View style={styles.listHeader}>
          <View style={styles.listHeaderCopy}>
            ...
            <Text style={styles.listSubtitle}>
              {visibleEntries.length} 项 · 按{sortLabel}排序
            </Text>
          </View>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="切换文件排序方式"
            disabled={!isConnected || isBusy}
        ...
        <FlatList
          ...
          contentContainerStyle={[
            styles.listContent,
            !visibleEntries.length && styles.emptyListContent,
          ]}
          refreshControl={
            <RefreshControl
          ...
          ListEmptyComponent={
            <View style={styles.emptyList}>
              <MaterialIcons
                name={isConnected ? "folder-open" : "wifi-off"}
                size={32}
                color="#91A5B3"
              />
          ...
          renderItem={({ item }) => {
            const itemSelected = isMultiSelecting
              ? multiSelected.some((entry) => entry.path === item.path)
            ...
            return (
              ...
              >
                ...
                style={({ pressed }) => [
                  styles.entry,
                  itemSelected && styles.entrySelected,
                  pressed && styles.pressed,
                ]}
              >
                <View
                  style={[
                    styles.entryIcon,
                    isDirectory && styles.directoryIcon,
                  ]}
                >
                  <MaterialIcons
                    name={fileIcon(item)}
                    size={21}
                    color={isDirectory ? "#007E7A" : "#5E7182"}
                  />
                </View>
                <View style={styles.entryCopy}>
                  <View style={styles.entryTitleRow}>
                    ...
                    {isDirectory ? (
                      <Text style={styles.entryKind}>文件夹</Text>
                    ) : null}
                  </View>
                ...
                {isMultiSelecting ? (
                  <MaterialIcons
                    name={
                      itemSelected ? "check-circle" : "radio-button-unchecked"
                    }
                    size={22}
                    color={itemSelected ? "#007E7A" : "#A0AFBA"}
                  />
                ) : isDirectory ? (
                  <MaterialIcons
                    name="chevron-right"
                  ...
                  <MaterialIcons
                    name={itemSelected ? "check-circle" : "more-horiz"}
                    size={21}
        ...
        {activeSelection.length ? (
          <View style={styles.selectionTray}>
            <View style={styles.selectionInfo}>
              <MaterialIcons name="checklist" size={19} color="#007E7A" />
              <View style={styles.selectionCopy}>
                <Text style={styles.selectionName} numberOfLines={1}>
                  {isMultiSelecting
                    ? `已选择 ${activeSelection.length} 项`
                    : activeSelection[0].name}
                </Text>
                <Text style={styles.selectionMeta} numberOfLines={1}>
                  {isMultiSelecting
                    ? "可批量复制、移动或删除"
                    : describeEntry(activeSelection[0])}
                </Text>
              </View>
              <Pressable
                accessibilityRole="button"
                accessibilityLabel="取消文件选择"
                onPress={clearSelection}
                style={styles.closeSelection}
              >
                <MaterialIcons name="close" size={19} color="#60758B" />
              </Pressable>
            </View>
            <View style={styles.selectionActions}>
              ...
              >
                <MaterialIcons name="content-copy" size={17} color="#007E7A" />
                <Text style={styles.actionText}>复制</Text>
              </Pressable>
              <Pressable
                accessibilityRole="button"
                onPress={() => stageClipboard("move")}
                disabled={isBusy}
                style={({ pressed }) => [
                  styles.actionButton,
                  isBusy && styles.disabled,
      ...
      >
        <View style={styles.searchCard}>
          <View style={styles.pathHeading}>
            <MaterialIcons name="search" size={19} color="#007E7A" />
            <Text style={styles.pathLabel}>搜索文件夹</Text>
            <Text style={styles.upText}>{visibleEntries.length} 项</Text>
          </View>
          <TextInput
            accessibilityLabel="按文件名筛选"
        ...
        <View style={styles.pathCard}>
          <View style={styles.pathHeading}>
            <MaterialIcons name="folder-open" size={19} color="#007E7A" />
            <Text style={styles.pathLabel}>当前位置</Text>
            <Pressable
              accessibilityRole="button"
              accessibilityLabel="返回上级文件夹"
              disabled={!isConnected || currentPath === "/" || isBusy}
              onPress={() => openDirectory(parentRemotePath(currentPath))}
              style={({ pressed }) => [
                styles.upButton,
                (!isConnected || currentPath === "/" || isBusy) &&
                  styles.disabled,
                pressed && styles.pressed,
              ]}
            >
              <MaterialIcons name="arrow-upward" size={17} color="#466075" />
              <Text style={styles.upText}>上级</Text>
            </Pressable>
          </View>
          <View style={styles.pathInputRow}>
            <TextInput
              accessibilityLabel="路由器文件路径"
            ...
            >
              <Text style={styles.goText}>前往</Text>
            </Pressable>
          </View>
        </View>
        ...
        <View style={styles.primaryTools}>
          ...
          >
            <MaterialIcons name="upload-file" size={19} color="#FFFFFF" />
            <Text style={styles.primaryToolText}>上传</Text>
          </Pressable>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="在当前文件夹新建文件夹"
            disabled={!isConnected || isBusy}
            onPress={() => showPrompt("folder")}
            style={({ pressed }) => [
              styles.secondaryTool,
              (!isConnected || isBusy) && styles.disabled,
              pressed && styles.pressed,
            ]}
          >
            <MaterialIcons name="create-new-folder" size={19} color="#007E7A" />
            <Text style={styles.secondaryToolText}>新建文件夹</Text>
          </Pressable>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={isMultiSelecting ? "结束多选" : "开始多选"}
            disabled={!isConnected || isBusy}
            onPress={() =>
              isMultiSelecting
                ? clearSelection()
                : (setSelected(null), setIsMultiSelecting(true))
            }
            style={({ pressed }) => [
              styles.secondaryTool,
              (!isConnected || isBusy) && styles.disabled,
          ...
          >
            <MaterialIcons
              name={isMultiSelecting ? "close" : "checklist"}
              size={19}
              color="#007E7A"
            />
            <Text style={styles.secondaryToolText}>
              {isMultiSelecting ? "结束多选" : "多选"}
            </Text>
          </Pressable>
          {clipboard ? (
            <Pressable
              accessibilityRole="button"
              accessibilityLabel="将剪贴板内容粘贴到当前文件夹"
              disabled={!isConnected || isBusy}
            ...
            >
              <MaterialIcons
                name={
                  clipboard.mode === "copy" ? "content-copy" : "drive-file-move"
                }
                size={18}
                color="#785000"
              />
              <Text style={styles.pasteToolText}>
                {clipboard.mode === "copy" ? "粘贴复制" : "粘贴移动"}
              </Text>
            </Pressable>
          ) : null}
        </View>

        {false && activeSelection.length ? (
          <View style={styles.selectionTray}>
            <View style={styles.selectionInfo}>
              <MaterialIcons
                name={
                  isMultiSelecting ? "checklist" : fileIcon(activeSelection[0])
                }
                size={19}
                color="#007E7A"
              />
          ...
          <View style={styles.promptModal}>
            ...
            <Text style={styles.modalDescription}>
              {promptKind === "permissions"
                ? "仅输入 3 或 4 位八进制权限。常用文件为 644，可执行脚本通常为 755。"
                : `位置：${currentPath}`}
            </Text>
            <TextInput
            ...
            <View style={styles.modalActions}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.confirmModal,
                  !promptValue.trim() && styles.disabled,
                  pressed && styles.pressed,
                ]}
              >
                <Text style={styles.confirmModalText}>确认</Text>
              </Pressable>
            </View>
          </View>
        </View>
      ...
      <Modal
        ...
        animationType="slide"
        visible={editor !== null}
        onRequestClose={() => !isSavingText && setEditor(null)}
      >
        <KeyboardAvoidingView
          ...
          <View style={styles.editorModal}>
            <View style={styles.editorHeader}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.closeEditor,
                  isSavingText && styles.disabled,
                  pressed && styles.pressed,
                ]}
              >
                <MaterialIcons name="close" size={22} color="#466075" />
              </Pressable>
            </View>
            <TextInput
...
const baseStyles = StyleSheet.create({
  ...
  navTitle: { color: "#203B55", fontSize: 17, fontWeight: "800" },
  target: {
    color: "#718398",
    fontSize: 11,
    marginTop: 2,
  ...
  platformText: { flex: 1, color: "#805B16", fontSize: 12, lineHeight: 18 },
  connectionCard: {
    marginHorizontal: 20,
    marginTop: 4,
    padding: 13,
  ...
  pathLabel: { flex: 1, color: "#304B64", fontSize: 12, fontWeight: "800" },
  upButton: {
    flexDirection: "row",
    alignItems: "center",
    gap: 3,
  ...
  upText: { color: "#466075", fontSize: 11, fontWeight: "700" },
  pathInputRow: { flexDirection: "row", gap: 8 },
  pathInput: {
    flex: 1,
    minHeight: 38,
    borderRadius: 9,
  ...
  goText: { color: "#007E7A", fontSize: 12, fontWeight: "800" },
  directorySection: { flex: 1, minHeight: 260 },
  bottomPanel: { maxHeight: 370 },
  bottomPanelContent: { paddingBottom: 24 },
  searchCard: {
    marginHorizontal: 20,
    marginTop: 12,
    padding: 12,
  ...
  sortButtonText: { color: "#007E7A", fontSize: 12, fontWeight: "800" },
  primaryTools: {
    paddingHorizontal: 20,
    paddingTop: 11,
    paddingBottom: 2,
  ...
  primaryToolText: { color: "#FFFFFF", fontSize: 12, fontWeight: "800" },
  secondaryTool: {
    minHeight: 38,
  ...
  secondaryToolText: { color: "#007E7A", fontSize: 12, fontWeight: "800" },
  pasteTool: {
    minHeight: 38,
    paddingHorizontal: 11,
    flexDirection: "row",
  ...
  pasteToolText: { color: "#785000", fontSize: 12, fontWeight: "800" },
  selectionTray: {
    marginHorizontal: 20,
    marginTop: 8,
    marginBottom: 8,
  ...
  selectionName: { color: "#203B55", fontSize: 13, fontWeight: "800" },
  selectionMeta: { color: "#60758B", fontSize: 11, marginTop: 2 },
  closeSelection: { padding: 4 },
  selectionActions: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 6,
  ...
  actionText: { color: "#007E7A", fontSize: 11, fontWeight: "800" },
  deleteAction: {
    minHeight: 31,
    paddingHorizontal: 8,
    flexDirection: "row",
  ...
  deleteActionText: { color: "#FFFFFF", fontSize: 11, fontWeight: "800" },
  list: {
    flex: 1,
    marginHorizontal: 20,
    marginBottom: 0,
  ...
  entrySelected: { backgroundColor: "#F0FAF8" },
  entryIcon: {
    width: 40,
    height: 40,
    borderRadius: 12,
  ...
  directoryIcon: { backgroundColor: "#E6F5F4" },
  ...
  entryMeta: { color: "#718398", fontSize: 11, marginTop: 4 },
  ...
  modalTitle: { color: "#203B55", fontSize: 18, fontWeight: "800" },
  modalDescription: {
    color: "#60758B",
    fontSize: 12,
    lineHeight: 18,
  ...
  cancelModalText: { color: "#466075", fontSize: 13, fontWeight: "800" },
  confirmModal: {
    minHeight: 38,
    paddingHorizontal: 15,
    borderRadius: 9,
  ...
  editorHeading: { flex: 1, minWidth: 0 },
  editorTitle: { color: "#203B55", fontSize: 16, fontWeight: "800" },
  editorPath: {
    color: "#718398",
    fontSize: 11,
    marginTop: 3,
  ...
  editorHint: { flex: 1, color: "#718398", fontSize: 11, lineHeight: 15 },
  saveButton: {
    minWidth: 80,
    minHeight: 38,
    paddingHorizontal: 12,
