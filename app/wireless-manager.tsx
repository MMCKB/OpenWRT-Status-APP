    ...
    {!hasRouter ? <EmptyState icon="router" title="尚未选择路由器" description="请先在“路由器”页面选择一台设备。" /> : !isSupported ? <EmptyState icon="android" title="需要 Android 应用" description="无线管理通过应用内 SSH 运行，请安装最新 Android APK。" /> : <>
      ...
      <SectionCard title="访客网络">
        <View style={styles.guestForm}>
          <Text style={[styles.helper, { color: colors.muted }]}>会使用第一个已发现的无线设备，创建或更新应用专用的 guest 配置段。</Text>
          <TextInput value={guestSsid} onChangeText={setGuestSsid} editable={!disabled} maxLength={32} placeholder="访客网络名称" placeholderTextColor={colors.muted} style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.background }]} />
          <View style={styles.passwordField}>
            <TextInput value={guestPassword} onChangeText={setGuestPassword} editable={!disabled} secureTextEntry={!isGuestPasswordVisible} autoCapitalize="none" maxLength={63} placeholder="访客网络密码（8–63 位）" placeholderTextColor={colors.muted} style={[styles.input, styles.passwordInput, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.background }]} />
            <Pressable accessibilityRole="button" accessibilityLabel={isGuestPasswordVisible ? "隐藏访客网络密码" : "显示访客网络密码"} accessibilityState={{ selected: isGuestPasswordVisible }} hitSlop={8} onPress={() => setIsGuestPasswordVisible((visible) => !visible)} style={({ pressed }) => [styles.passwordToggle, pressed && styles.pressed]}>
              <MaterialIcons name={isGuestPasswordVisible ? "visibility-off" : "visibility"} size={21} color={colors.muted} />
            </Pressable>
          </View>
          <Pressable accessibilityRole="button" disabled={disabled || guestPassword.trim().length < 8 || !networks.length} onPress={createGuest} style={({ pressed }) => [styles.guestButton, { backgroundColor: colors.primary }, pressed && styles.pressed, (disabled || guestPassword.trim().length < 8 || !networks.length) && styles.disabled]}><MaterialIcons name="add-circle-outline" size={19} color="#FFFFFF" /><Text style={styles.guestButtonText}>创建或更新访客网络</Text></Pressable>
          {guestQr ? <View style={[styles.qrArea, { backgroundColor: "#FFFFFF" }]}><QRCode value={guestQr} size={156} color="#12313A" backgroundColor="#FFFFFF" /><Text style={styles.qrCaption}>访客扫描此二维码即可连接</Text></View> : <Text style={[styles.helper, { color: colors.muted }]}>填写至少 8 位密码后，将显示可扫描的离线二维码。</Text>}
        </View>
      </SectionCard>
    </>}
