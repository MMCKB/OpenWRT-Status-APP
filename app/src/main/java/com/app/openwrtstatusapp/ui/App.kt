...
private val OpenWrtBlue = Color(0xFF2274E5)
private val OpenWrtDark = Color(0xFF0B1220)

@Composable
fun OpenWrtTheme(content: @Composable () -> Unit) = MaterialTheme(
    colorScheme = androidx.compose.foundation.isSystemInDarkTheme().let { dark ->
        if (dark) androidx.compose.material3.darkColorScheme(primary = Color(0xFF8AB4FF), surface = OpenWrtDark)
        else androidx.compose.material3.lightColorScheme(primary = OpenWrtBlue, surface = Color(0xFFF7F9FC))
    }, content = content,
)
...
fun OpenWrtApp(vm: RouterViewModel = viewModel()) {
    ...
    Scaffold(
        topBar = { CenterAlignedTopAppBar(title = { Text(detail?.title ?: "OpenWrt 状态") }, navigationIcon = { if (detail != null) IconButton({ detail = null }) { Icon(Icons.Default.Close, "返回") } }, actions = { IconButton({ showRouters = true }) { Icon(Icons.Default.Settings, "路由器列表") }; IconButton({ showRouterEditor = true }) { Icon(Icons.Default.Add, "添加路由器") } }) },
        bottomBar = { if (detail == null) NavigationBar { listOf("状态" to Icons.Default.Home, "网络" to Icons.Default.NetworkCheck, "服务" to Icons.Default.Settings, "工具" to Icons.Default.Build).forEach { (name, icon) -> NavigationBarItem(selected = tab == name, onClick = { tab = name }, icon = { Icon(icon, name) }, label = { Text(name) }) } } },
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            if (selected == null) EmptyState { showRouterEditor = true }
                ...
                else -> ToolsPage(busy, output, vm::nat, toolFeatures, onFeature = { detail = it }, terminal = { showTerminal = true }, files = { showFiles = true }, packages = { showPackages = true })
            }
        }
    }
...
@Composable private fun Dashboard(profile: RouterProfile, status: RouterStatus?, busy: Boolean, refresh: (RouterProfile) -> Unit, connect: (RouterProfile) -> Unit, disconnect: (RouterProfile) -> Unit, showTerminal: () -> Unit, showFiles: () -> Unit, showPackages: () -> Unit) = LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
    item { Card { Column(Modifier.padding(18.dp)) { Row(verticalAlignment = Alignment.CenterVertically) { Column(Modifier.weight(1f)) { Text(profile.name, style = MaterialTheme.typography.titleLarge); Text(profile.baseUrl, color = MaterialTheme.colorScheme.onSurfaceVariant) }; Button({ refresh(profile) }, enabled = !busy) { Icon(Icons.Default.Refresh, null); Spacer(Modifier.width(5.dp)); Text("刷新") } }; Spacer(Modifier.height(12.dp)); val system = status?.system; Info("主机名", system?.hostname ?: "等待刷新"); Info("型号", system?.model ?: "—"); Info("固件", system?.firmware ?: "—"); Info("内核", system?.kernel ?: "—"); Info("CPU", system?.cpuModel ?: "—"); Info("CPU 温度", system?.cpuTemperature ?: "—"); Info("WiFi 温度", system?.wifiTemperature ?: "—"); Info("运行时间", formatUptime(system?.uptimeSeconds)); status?.error?.let { Text(it, color = MaterialTheme.colorScheme.error) } } } }
