...
export default function WakeOnLanScreen() {
  ...
  return <ManagementShell title="网络唤醒" description="选择已发现设备后，由 OpenWrt 路由器发送 Wake-on-LAN 广播包。"><SectionCard title="选择要唤醒的设备"><View style={styles.form}><View style={styles.sectionHeader}><Text style={[styles.label, { color: colors.foreground }]}>已发现客户端</Text><Pressable accessibilityRole="button" onPress={() => void refreshClients()} disabled={isRunning || !hasRouter || !isSupported} style={({ pressed }) => [styles.refreshButton, { borderColor: colors.border }, pressed && styles.pressed]}><MaterialIcons name="refresh" size={17} color={colors.primary} /><Text style={[styles.refreshText, { color: colors.primary }]}>重新扫描</Text></Pressable></View>{clients.length ? <View style={[styles.clientList, { borderColor: colors.border }]}>{clients.map((client, index) => { const selected = selectedMac === client.mac; return <Pressable key={client.mac} accessibilityRole="radio" accessibilityState={{ selected }} onPress={() => setSelectedMac(client.mac)} style={({ pressed }) => [styles.clientRow, index > 0 && { borderTopWidth: 1, borderTopColor: colors.border }, selected && { backgroundColor: colors.background === "#102A43" ? "#164B3B" : "#E8F7F1" }, pressed && styles.pressed]}><View style={styles.clientInfo}><Text numberOfLines={1} style={[styles.clientName, { color: colors.foreground }]}>{client.hostname ?? "未命名设备"}</Text><Text numberOfLines={1} style={[styles.clientMeta, { color: colors.muted }]}>{client.ipv4 ?? "未取得 IP"} · {client.mac}</Text></View><MaterialIcons name={selected ? "radio-button-checked" : "radio-button-unchecked"} size={22} color={selected ? colors.primary : colors.muted} /></Pressable>; })}</View> : <Text style={[styles.emptyText, { color: colors.muted }]}>暂未读取到设备，请重新扫描。</Text>}<Pressable accessibilityRole="button" disabled={disabled} onPress={sendWakePacket} style={({ pressed }) => [styles.button, { backgroundColor: colors.primary }, pressed && styles.pressed, disabled && styles.disabled]}>{isRunning ? <ActivityIndicator color="#FFFFFF" /> : <><MaterialIcons name="power-settings-new" size={18} color="#FFFFFF" /><Text style={styles.buttonText}>{selectedClient ? `唤醒 ${selectedClient.hostname ?? selectedClient.mac}` : "请选择设备"}</Text></>}</Pressable></View></SectionCard><ToolNotice><Text style={[styles.notice, { color: colors.muted }]}>如提示缺少工具，请在路由器的软件包管理中安装 etherwake、wakeonlan 或 wol。部分无线网卡、关机状态或跨网段场景不支持网络唤醒。</Text></ToolNotice>{error ? <ToolNotice><Text style={[styles.notice, { color: colors.error }]}>{error}</Text></ToolNotice> : null}{notice ? <ToolNotice><Text style={[styles.notice, { color: colors.foreground }]}>{notice}</Text></ToolNotice> : null}</ManagementShell>;
}
